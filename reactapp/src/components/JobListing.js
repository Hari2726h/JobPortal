...
const JobListing = ({ setJobs, jobs = [], onSelectJob }) => {
    ...
    return (
        <div data-testid="job-listing" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: '15px' }}>
            {localJobs.map((job) => (
                <div key={job.id} data-testid={`job-item-${job.id}`}
                 onClick={() => onSelectJob && onSelectJob(job.id)} 
                 style={{ cursor: 'pointer', marginBottom: '10px', border: '1px solid #ddd', borderRadius: '8px', padding: '15px',
                  boxShadow: '0 2px 6px rgba(0,0,0,0.1)', transition: 'transform 0.2s ease', backgroundColor: '#fff' }}
